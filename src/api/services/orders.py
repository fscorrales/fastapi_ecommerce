...
class OrdersService:
    ...
    @classmethod
    def update_cart(cls, customer_id: PydanticObjectId, product: OrderProducts):
        try:
            if document := cls.collection.find_one(
                {"customer_id": customer_id, "status": "shopping"}
            ):
                return cls.update_order_product(
                    document.get("_id"), product, opt="update"
                )
            else:
                raise HTTPException(
                    status_code=status.HTTP_404_NOT_FOUND,
    ...
    def update_order_product(
        ...
        opt: Literal["add", "remove", "update"] = "add",
    ):
        try:
            if opt == "add":
                if cls.collection.find_one(
                ...
                document = cls.collection.find_one_and_update(
                    {"_id": id},
                    {"$set": {"order_products.$[elem].quantity": product.quantity}},
                    array_filters=[{"elem.product_id": product.product_id}],
                    return_document=True,
                )
            if document:
